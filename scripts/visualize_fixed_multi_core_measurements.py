...
num_cores = [24.0, 48.0]
for filename in os.listdir("./scripts/23-03-25/benchmark_mmap_based_fixed_multi_core"):
    if filename.endswith(".json"):
        with open(f"./scripts/23-03-25/benchmark_mmap_based_fixed_multi_core/{filename}") as f:
            #load json
            data = json.load(f)
            ...
            data_df = data_df.append({"num_cores": int(num_cores), "num_clients": int(num_clients), "type": type, "latency": latency_all}, ignore_index=True)

for num_core in data_df['num_cores'].unique():
    plot_data = data_df[data_df['num_cores'] == num_core]
    benchmark_results = sns.lineplot(data=plot_data, x="num_clients", y="latency", hue="type", marker='o', linestyle='--')

    benchmark_results.set(
        xlabel="#Clients", ylabel="Latency in ms/iter (Sum over all Queries)", title=f"Comparison of MMAP-based Hyrise vs. Hyrise Master in \nSum of Average Latency over All Queries Depending on #Clients."
    )
