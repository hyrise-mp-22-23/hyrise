...
for num_core, do_warmup in itertools.product(num_cores, num_clients, warmup):
    print(f"Num cores: {num_core}, Num clients: {num_clients}, Warmup: {do_warmup}")
    benchmark_command = [
        'numactl',
        '-m',
        '0',
        ...
        '1200',
        '-o',
        f'benchmark_hyrise_master_{num_core}_cores_{num_clients}_clients_{"with_warmup" if do_warmup else ""}.json'
    ]
    if (do_warmup):
