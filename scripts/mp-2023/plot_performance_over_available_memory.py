            ...
            if "master" in filename:
                ...
                for limit_size in limit_sizes:
                    data_df = data_df.append(
                        {"memory": int(limit_size), "type": type, "latency": latency_all}, ignore_index=True
                    )
            else:
                type = "mmap-based"
                ...
                if memory_key in pgfault_data:
                    ...
                    execution_pgfaults = parse_occurred_pagefaults(after_memory_stats) - parse_occurred_pagefaults(
                        before_memory_stats
                    )
                # append data to data frame
                data_df = data_df.append(
                    {"memory": memory, "type": type, "latency": int(latency_all), "pgfaults": execution_pgfaults},
                    ignore_index=True,
                )


benchmark_results = sns.lineplot(data=data_df, x="memory", y="latency", hue="type", marker="o", linestyle="--")
benchmark_results_second = sns.lineplot(
    data=data_df, x="memory", y="pgfaults", color="red", marker="o", linestyle="dotted", ax=benchmark_results.twinx()
)

benchmark_results.set(
    xlabel="Available RAM in GB",
    ylabel="Latency in ms/iter (Sum over all Queries)",
    title=f"MMAP-Based Single-Threaded Hyrise\nSum of Average Latency over all Queries Depending on Memory Limitation.",
)
