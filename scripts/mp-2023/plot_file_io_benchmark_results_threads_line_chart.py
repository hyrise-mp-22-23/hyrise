...
if len(sys.argv) != 4:
    print(
        "You did not specify the benchmark type. If you want only fio read or only fio write benchmarks plotted, please do so."
    )

benchmark_df = pd.read_csv(sys.argv[1])
fio_df = pd.read_csv(sys.argv[2])

benchmark_df[["fixture", "io_type", "filesize_mb", "threads", "real_time_appendix"]] = benchmark_df["name"].str.split(
    "/", 4, expand=True
)
fio_df[["fixture", "io_type", "filesize_mb", "threads", "real_time_appendix"]] = fio_df["name"].str.split(
    "/", 4, expand=True
)

# drop rows containing pre-calculated statistical data (if provided)
benchmark_df.drop(
    benchmark_df[benchmark_df.real_time_appendix.str.contains("_mean|_median|_stddev|_cv")].index, inplace=True
)
...
for filesize in benchmark_df["filesize_mb"].unique():
    ...
    benchmark_results.set(
        ...
        title=f"Different I/O method speed dependent on threads for {filesize}MB",
    )
