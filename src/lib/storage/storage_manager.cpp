...
std::pair<uint32_t, uint32_t> StorageManager::persist_chunk_to_file(const std::shared_ptr<Chunk> chunk,
                                                                    ChunkID chunk_id, const std::string& file_name) {
...
void StorageManager::replace_chunk_with_persisted_chunk(const std::shared_ptr<Chunk>& chunk, ChunkID chunk_id,
  ...
  get_map_for_chunk(chunk, table_name, chunk_start_offset, chunk_bytes, table_persistence_file, chunk_id);
...
std::shared_ptr<Chunk> StorageManager::map_chunk_from_disk(const uint32_t chunk_offset_begin,
