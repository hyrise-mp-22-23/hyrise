...
int main(int argc, char* argv[]) {
  ...
  cli_options.add_options()
    ...
    ("use_storage_json", "Use storage.json to load TPCH data using the fileformat for chunks that hold their data with mmap."); // NOLINT
  // clang-format on
  ...
  use_storage_json = cli_parse_result["use_storage_json"].as<bool>();
  ...
  std::cout << "- Using storage.json: " << (use_storage_json ? "yes" : "no") << std::endl;
  ...
  context.emplace("use_storage_json", use_storage_json);
  config->encoding_config = EncodingConfig();
  config->use_storage_json = use_storage_json;
