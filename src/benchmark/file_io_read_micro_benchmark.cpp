...
void FileIOMicroReadBenchmarkFixture::libaio_sequential_read_multi_threaded(benchmark::State& state,
  ...
  for (auto _ : state) {
    ...
    for (auto index = size_t{0}; index < thread_count; ++index) {
      ...
      threads[index] = (std::thread(read_data_using_libaio, from, to, filedescriptors[index], std::data(read_data), std::ref(threads_ready_to_be_executed)));
    }
  ...
  for (auto _ : state) {
    ...
    for (auto index = size_t{0}; index < thread_count; ++index) {
      ...
      threads[index] = (std::thread(read_data_randomly_using_libaio, from, to, filedescriptors[index],
                                    std::data(read_data), random_indices, std::ref(threads_ready_to_be_executed)));
    }
